...
Clazz.load (null, "JS.MathExt", ["java.lang.Float", "$.Number", "java.util.Date", "$.Hashtable", "$.Random", "JU.AU", "$.BS", "$.CU", "$.Lst", "$.M3", "$.M4", "$.Measure", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.VDW", "J.i18n.GT", "JM.BondSet", "JS.SV", "$.ScriptParam", "$.T", "JU.BSUtil", "$.Escape", "$.JmolMolecule", "$.Logger", "$.Parser", "$.Point3fi"], function () {
...
case 135266306:
case 269484096:
return this.evaluateArray (mp, args, false);
case 135266307:
case 135270418:
...
case 135266310:
case 1276252167:
return this.evaluateConnected (mp, args, tok);
case 135402505:
...
case 1276117505:
return this.evaluateDotDist (mp, args, 0);
case 1276118018:
if (op.tok == 269484241) return this.evaluateDotDist (mp, args, op.intValue);
case 135266305:
case 1746538509:
...
case 1288701959:
case 1826248716:
return this.evaluateFormat (mp, op.intValue, args, tok == 1826248716);
case 135368713:
return this.evaluateUserFunction (mp, op.value, args, op.intValue, op.tok == 269484241);
case 1276117018:
case 1276121098:
return this.evaluateGetProperty (mp, args, tok == 1276117018, op.tok == 269484241);
case 137363467:
...
var key = (args.length == 4 ? JS.SV.sValue (args[3]) : null);
var data;
var maps = null;
if (isListf) {
data = x1.value;
...
var v1 = f0 + df * bin;
var v2 = v1 + df;
map.put ("_binMin", JS.SV.newV (3, Float.$valueOf (bin == 0 ? -3.4028235E38 : v1)));
map.put ("_binMax", JS.SV.newV (3, Float.$valueOf (bin == nbins - 1 ? 3.4028235E38 : v2)));
}}
...
if (bs1 != null) smiles1 = this.e.getSmilesExt ().getSmilesMatches ("", null, bs1, null, false, true, false);
var check;
if (bs2 == null) {
check = (this.vwr.getSmilesMatcher ().areEqual (smiles2, smiles1) > 0);
} else {
check = ((this.e.getSmilesExt ().getSmilesMatches (smiles1, null, bs2, null, false, true, false)).nextSetBit (0) >= 0);
}if (!check) {
...
var ret = this.e.getSmilesExt ().getSmilesMatches ("/nostereo/" + smiles1, null, bs2, null, false, true, false);
...
stddev = this.e.getSmilesExt ().getSmilesCorrelation (bs1, bs2, smiles1, null, null, null, null, false, null, null, false, 65536);
...
if (isPolyhedron) sOpt = (args.length > 4 ? JS.SV.sValue (args[4]) : null);
...
if (!isMap && !isSmiles) return false;
sOpt = "/noaromatic" + (allMaps || bestMap ? "/" : " nostereo/") + this.e.getSmilesExt ().getSmilesMatches ((hMaps ? "H" : ""), null, bs1, null, false, true, false);
} else {
allMaps = true;
}stddev = this.e.getSmilesExt ().getSmilesCorrelation (bs1, bs2, sOpt, ptsA, ptsB, m, null, isMap, null, null, bestMap, (isSmiles ? 65536 : 131072) | (!allMaps && !bestMap ? 4194304 : 0));
if (isMap) {
...
var isBondCount = (tok == 1229980163);
if (args.length > 5 || isBondCount && args.length != 1) return false;
var min = -2147483648;
var max = 2147483647;
var fmin = 0;
...
}var data =  Clazz.newArray (-1, [Integer.$valueOf (nv), smiles, null]);
this.vwr.shm.getShapePropertyData (21, "centers", data);
return mp.addXBs (data[2] == null ?  new JU.BS () : data[2]);
case 1229980163:
var x1 = mp.getX ();
if (x1.tok != 10) return false;
atoms1 = JS.SV.bsSelectVar (x1);
atoms2 = JS.SV.bsSelectVar (args[0]);
var list =  new JU.Lst ();
...
return mp.addX (JS.SV.newV (10, JM.BondSet.newBS (bsBonds, this.vwr.ms.getAtomIndices (this.vwr.ms.getAtoms (1678770178, bsBonds)))));
...
var f1 = this.vwr.getDataFloat (selected, null);
if (f1 == null) return mp.addXStr ("");
var f2 = (type.indexOf ("property_") == 0 ? this.vwr.getDataFloat (type, null) : null);
if (f2 != null) {
...
Clazz.defineMethod (c$, "evaluateDotDist", 
 function (mp, args, intValue) {
var isDist = (intValue != 0);
switch (args.length) {
case 1:
break;
case 2:
...
var x1 = mp.getX ();
var x2 = args[0];
var pt2 = (x2.tok == 7 ? null : mp.ptValue (x2));
...
}var pt1 = mp.ptValue (x1);
var plane1 = mp.planeValue (x1);
var f = NaN;
try {
if (isDist) {
if (plane2 != null && args.length == 2) f = JU.Measure.directedDistanceToPlane (pt1, plane2, JS.SV.ptValue (args[1]));
...
if (args.length > 2) {
var pta = mp.ptValue (args[0]);
var ptb = mp.ptValue (args[1]);
...
}var bs = (Clazz.instanceOf (args[0].value, JU.BS) ? args[0].value : this.vwr.ms.getAtoms (1095763988,  new Integer (args[0].asInt ())));
switch (tok) {
case 135266320:
...
var isSearch = !isList && sFind.equalsIgnoreCase ("SMARTS");
...
var isON = !isList && (args[args.length - 1].tok == 1048589);
try {
if (isChemical) {
...
var firstMatchOnly = (args.length < 4 || !isON);
if (args.length > 3) isAll = (args[2].tok == 1048589);
ret = this.e.getSmilesExt ().getSmilesMatches (flags, smiles, null, null, isSearch, !isAll, firstMatchOnly);
}break;
case 10:
...
if (isSequence) return mp.addXStr (this.vwr.getSmilesOpt (x1.value, -1, -1, 256 | (isAll ? 259 : 0)));
if (isSmiles || isSearch) sFind = flags;
var bsMatch3D = bs2;
if (asBonds) {
var map = this.vwr.getSmilesMatcher ().getCorrelationMaps (sFind, this.vwr.ms.at, this.vwr.ms.ac, x1.value, (isSmiles ? 65536 : 131072) | 4194304);
ret = (map.length > 0 ? this.vwr.ms.getDihedralMap (map[0]) :  Clazz.newIntArray (0, 0));
} else {
ret = this.e.getSmilesExt ().getSmilesMatches (sFind, null, x1.value, bsMatch3D, !isSmiles, !isAll, false);
}break;
}
...
Clazz.defineMethod (c$, "evaluateGetProperty", 
 function (mp, args, isAuxiliary, isAtomProperty) {
var pt = 0;
if (isAuxiliary && args.length != 1) return false;
...
if (isAtomProperty) {
x = mp.getX ();
if (x.tok != 10) return mp.addXObj (this.vwr.extractProperty (x, propertyName, -1));
}if (isAtomProperty && !isAuxiliary && !lc.startsWith ("bondinfo") && !lc.startsWith ("atominfo")) propertyName = "atomInfo." + propertyName;
var propertyValue = "";
...
if (propertyName.equalsIgnoreCase ("bondInfo") && args.length > pt && args[pt].tok == 10) propertyValue =  Clazz.newArray (-1, [propertyValue, JS.SV.bsSelectVar (args[pt])]);
break;
case 6:
case 4:
...
var bs = JS.SV.bsSelectVar (x);
var iAtom = bs.nextSetBit (0);
if (iAtom < 0) return mp.addXStr ("");
propertyValue = bs;
}if (isAuxiliary) propertyName = "auxiliaryInfo.models." + propertyName;
propertyName = JU.PT.rep (propertyName, ".[", "[");
var property = this.vwr.getProperty (null, propertyName, x == null ? propertyValue : x);
...
}, "JS.ScriptMathProcessor,~A,~B,~B");
Clazz.defineMethod (c$, "evaluateFormat", 
 function (mp, intValue, args, isLabel) {
var x1 = (args.length < 2 || intValue == 1288701959 ? mp.getX () : null);
...
var o = JS.SV.format (args, pt, false);
...
args2[1] = a.get (i);
sa[i] = JS.SV.format (args2, pt, true).toString ();
}
return mp.addXAS (sa);
...
var list =  new JU.Lst ();
var map = listIn.get (i).getMap ();
for (var j = 0, n1 = formatList.size (); j < n1; j++) {
var v = map.get (JS.SV.sValue (formatList.get (j)));
...
}if (x2.tok != 1048589) return false;
var l = x1.getList ();
...
for (var i = 0; i < nPoints; i++) {
if ((pts[i] = mp.ptValue (args[i])) == null) return false;
}
switch (nPoints) {
case 2:
...
pt3 = (args.length > 2 && (args[2].tok == 10 || args[2].tok == 8) ? mp.ptValue (args[2]) : null);
norm = JU.V3.newV (pt2);
if (pt3 == null) {
plane =  new JU.P4 ();
...
var ptref = (args.length == 4 ? mp.ptValue (args[3]) : null);
...
case 2:
var pt3 = JS.SV.ptValue (args[0]);
if (pt3 == null) return false;
if (args[1].tok == 1048588) {
...
}if ((pt0 = mp.ptValue (args[0])) == null || tok != 135270418 && args[1].tok == 8) return false;
break;
case 3:
...
Clazz.defineMethod (c$, "evaluateScript", 
 function (mp, args, tok) {
if ((tok == 135266829 || tok == 135270926 || tok == 135287308) && args.length != 1 || args.length == 0 || args.length > 2) return false;
var s = JS.SV.sValue (args[0]);
var sb =  new JU.SB ();
switch (tok) {
case 135266829:
...
Clazz.defineMethod (c$, "evaluateString", 
 function (mp, tok, args) {
var x = mp.getX ();
var sArg = (args.length > 0 ? JS.SV.sValue (args[0]) : tok == 1276117512 ? "" : "\n");
switch (args.length) {
case 0:
case 1:
break;
...
if (x.tok == 7 && tok != 1276117512 && (tok != 1276118021 || args.length == 2)) {
mp.addX (x);
return this.evaluateList (mp, tok, args);
}var s = (tok == 1276118021 && x.tok == 10 || tok == 1276117512 && x.tok == 7 ? null : JS.SV.sValue (x));
switch (tok) {
case 1276118021:
if (x.tok == 10) {
...
bs = this.vwr.getSmilesMatcher ().getSubstructureSet (pattern, this.vwr.ms.at, this.vwr.ms.ac, bsSelected, (tok == 135267336 ? 65536 : 131072));
} catch (ex) {
if (Clazz.exceptionOf (ex, Exception)) {
this.e.evalError (ex.getMessage (), null);
} else {
...
var pt = (args.length > 1 ? mp.ptValue (args[1]) : null);
if (args.length == 2 && pt != null) return mp.addXObj (this.vwr.ms.getSymTemp (false).getSymmetryInfoAtom (this.vwr.ms, bs, xyz, iOp, pt, null, null, 135266320));
var desc = (args.length == 1 ? "matrix" : JS.SV.sValue (args[args.length - 1])).toLowerCase ();
var tok = 135176;
...
tok = 12289;
}return mp.addXObj (this.vwr.ms.getSymTemp (false).getSymmetryInfoAtom (this.vwr.ms, bs, xyz, iOp, pt, null, desc, tok));
}, "JS.ScriptMathProcessor,~A,~B");
...
if (isSelector) {
return mp.addXObj (this.e.getBitsetProperty (JS.SV.bsSelectVar (x1), tok, null, null, x1.value,  Clazz.newArray (-1, [name, params]), false, x1.index, false));
...
var isWithinUnitcell = false;
var isDistance = (isVdw || tok == 3 || tok == 2);
var rd = null;
switch (tok) {
case 1048580:
return (i == 3 && Clazz.instanceOf (args[1].value, JU.BS) && Clazz.instanceOf (args[2].value, JU.BS) && mp.addXBs (this.vwr.getBranchBitSet ((args[2].value).nextSetBit (0), (args[1].value).nextSetBit (0), true)));
case 135267336:
case 1238369286:
case 135267335:
var bsSelected = null;
...
return mp.addXObj (this.e.getSmilesExt ().getSmilesMatches (JS.SV.sValue (args[1]), null, bsSelected, null, tok == 135267335, mp.asBitSet, false));
}
...
i = 0;
if (args.length > 2 && JS.SV.sValue (args[2]).equalsIgnoreCase ("unitcell")) tok = 1614417948;
break;
case 4:
var s = JS.SV.sValue (args[1]);
...
return mp.addXStr (this.e.getCmdExt ().dispatch (135270422, true, args));
}, "JS.ScriptMathProcessor,~A");
