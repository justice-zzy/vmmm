...
Clazz.load (["JS.ScriptExt"], "JS.CmdExt", ["java.lang.Boolean", "$.Float", "$.Long", "java.util.Hashtable", "JU.AU", "$.BS", "$.Base64", "$.Lst", "$.M3", "$.M4", "$.Measure", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.STER", "$.VDW", "J.i18n.GT", "JM.Atom", "$.AtomCollection", "$.BondSet", "$.LabelToken", "JS.SV", "$.ScriptCompiler", "$.ScriptError", "$.ScriptEval", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.ScriptParam", "$.T", "JU.BSUtil", "$.BoxInfo", "$.C", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.Parser", "$.Point3fi", "JV.FileManager", "$.JC", "$.StateManager", "$.Viewer"], function () {
...
case 1073742102:
if (!this.chk) {
if (eval.tokAt (2) == 1276252167) this.setShapeProperty (21, "symmetry", null);
 else this.showString (this.vwr.ms.calculatePointGroup (this.vwr.bsA ()));
}return;
case 1112539150:
this.checkLength (2);
if (!this.chk) {
...
}var stddev = eval.getSmilesExt ().getSmilesCorrelation (bsFrom, bsTo, strSmiles, null, null, m4, null, false, null, center, false, isSmiles ? 65536 : 131072);
...
this.vwr.addStateScript ("configuration " + bsAtoms + " " + n + ";", true, false);
...
var maps = null;
try {
maps = this.vwr.getSmilesMatcher ().getCorrelationMaps (smarts, atoms, ac, this.vwr.bsA (), 131072);
} catch (ex) {
if (Clazz.exceptionOf (ex, Exception)) {
eval.evalError (ex.getMessage (), null);
} else {
...
}if (report) eval.report (J.i18n.GT.o (J.i18n.GT._ ("{0} new bonds; {1} modified"),  Clazz.newArray (-1, [Integer.$valueOf (nNew), Integer.$valueOf (nModified)])));
}, "~N");
...
isQ = (this.tokAt (eval.iToken + 1) == 1048589);
break;
case 1073741824:
...
i = eval.iToken;
if (!this.chk) list.addLast ( Clazz.newArray (-1, [Integer.$valueOf (135266320), Float.$valueOf (timeSec), pt]));
continue;
case 528432:
switch (this.getToken (++i).tok) {
case 1112541205:
...
}if (!this.chk) list.addLast ( Clazz.newArray (-1, [Integer.$valueOf (269484210), Float.$valueOf (timeSec), Float.$valueOf (x), Float.$valueOf (y)]));
continue;
case 269484208:
continue;
case 1113200654:
...
list.addLast ( Clazz.newArray (-1, [Integer.$valueOf (1113200654), Float.$valueOf (timeSec), pathGuide]));
continue;
}break;
case 1073742084:
var path;
...
if ((propToks[i] = this.tokAt (eval.iToken)) != 1716520985 && !JS.T.tokAttr (propToks[i], 1078984704)) this.invArg ();
...
if (tok == 1716520985) {
dataX = this.getBitsetPropertyFloat (bs, propToks[0] | 224, propToks[0] == 1716520985 ? props[0] : null, (minXYZ == null ? NaN : minXYZ.x), (maxXYZ == null ? NaN : maxXYZ.x));
propData[0] = props[0] + " " + JU.Escape.eAF (dataX);
if (props[1] != null) {
dataY = this.getBitsetPropertyFloat (bs, propToks[1] | 224, propToks[1] == 1716520985 ? props[1] : null, (minXYZ == null ? NaN : minXYZ.y), (maxXYZ == null ? NaN : maxXYZ.y));
propData[1] = props[1] + " " + JU.Escape.eAF (dataY);
}if (props[2] != null) {
dataZ = this.getBitsetPropertyFloat (bs, propToks[2] | 224, propToks[2] == 1716520985 ? props[2] : null, (minXYZ == null ? NaN : minXYZ.z), (maxXYZ == null ? NaN : maxXYZ.z));
...
if (JS.T.tokAttr (propToks[0], 1095761920)) {
factors.x = 1;
center.x = 0;
} else if (factors.x > 0.1 && factors.x <= 10) {
factors.x = 1;
}if (JS.T.tokAttr (propToks[1], 1095761920)) {
factors.y = 1;
center.y = 0;
} else if (factors.y > 0.1 && factors.y <= 10) {
factors.y = 1;
}if (JS.T.tokAttr (propToks[2], 1095761920)) {
factors.z = 1;
center.z = 0;
...
decimalPropertyName = JS.T.nameOf (eval.theTok);
...
case 1073742060:
propertyName = "token";
propertyValue = Integer.$valueOf (this.e.theTok);
break;
case 1073742044:
case 1073741933:
case 1073741956:
...
if (s.length > 0 && s.charAt (0) != '.') type = s.toUpperCase ();
...
} else if (data === "SDF" || data === "MOL" || data === "V2000" || data === "V3000" || data === "CD" || data === "JSON" || data === "XYZ" || data === "XYZRN" || data === "XYZVIB" || data === "CML") {
...
if (this.chk) return;
try {
if (tok != 135267336) msg = this.vwr.getDataBaseName (null);
if (msg != null && (msg.startsWith ("$") || msg.startsWith (":"))) {
msg = msg.substring (1);
} else {
msg = null;
}if (msg == null) msg = this.vwr.getSmiles (null);
} catch (ex) {
if (Clazz.exceptionOf (ex, Exception)) {
msg = ex.getMessage ();
} else {
...
if (name.startsWith ("/") && (len = this.slen) == 3) {
if (!this.chk) {
...
case 1641025539:
if ((len = this.slen) == 2) {
if (!this.chk) msg = this.vwr.ms.getProteinStructureState (this.vwr.bsA (), 135270926);
break;
...
len = this.slen;
if (!this.chk) this.showString (this.vwr.ms.getPointGroupAsString (this.vwr.bsA (), false, "show:" + typ, 0, 0));
return;
case 1089470479:
...
}s = this.vwr.ms.getInfo (this.vwr.am.cmi, "unitcell_" + s);
this.showString (s);
}newUC = s;
break;
case 135180:
...
eval.setObjectMad (33, "unitCell", mad);
...
var data = (property == null ? this.e.getBitsetProperty (bs, tok, null, null, null, null, false, 2147483647, false) : this.vwr.getDataFloat (property, bs));
