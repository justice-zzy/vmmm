...
Clazz_declarePackage ("JS");
Clazz_load (["J.api.SmilesMatcherInterface"], "JS.SmilesMatcher", ["JU.AU", "$.BS", "$.PT", "JS.InvalidSmilesException", "$.SmilesAtom", "$.SmilesBond", "$.SmilesGenerator", "$.SmilesParser", "JU.BSUtil", "$.Node", "$.Point3fi"], function () {
c$ = Clazz_declareType (JS, "SmilesMatcher", null, J.api.SmilesMatcherInterface);
Clazz_defineMethod (c$, "checkFlag", 
 function (flags, flag) {
...
atoms[i].elementNumber = (pt == null ? -2 : Clazz_instanceOf (pt, JU.Node) ? (pt).getElementNumber () : Clazz_instanceOf (pt, JU.Point3fi) ? (pt).sD : -2);
atoms[i].index = i;
}
var nBonds = 0;
...
break;
case 1:
atom2 = this.getHydrogens (sAtom.getMatchingAtom (), null);
if (sAtom.isFirst) {
var a = atom2;
atom2 = atom1;
atom1 = a;
...
if (this.isSmilesFind && !this.setSmilesCoordinates (atom0, sAtom, sAtom2,  Clazz_newArray (-1, [atom1, atom2, atom3, atom4, atom5, atom6]))) return false;
...
function (atoms, ac, bsSelected, explicitH, topologyOnly, getAromatic) {
...
return this.getSmilesComponent (atoms[i], bsSelected, true, false);
}, "~A,~N,JU.BS,~B,~B,~B");
...
s = this.getSmilesComponent (a, bs, false, true);
...
Clazz_defineMethod (c$, "getSmilesComponent", 
 function (atom, bs, allowBioResidues, allowConnectionsToOutsideWorld) {
...
if (this.topologyOnly) sb.append ("*");
 else if (isExtension && groupType.length != 0 && atomName.length != 0) this.addBracketedBioName (sb, atom, "." + atomName);
...
if (s == null) {
ht.put (key,  Clazz_newArray (-1, [s = this.getRingPointer (++this.nPairs), Integer.$valueOf (i1)]));
...
return (count == valence ? sym : "[" + (isotopeNumber <= 0 ? "" : "" + isotopeNumber) + sym + (charge < 0 && charge != -2147483648 ? "" + charge : charge > 0 ? "+" + charge : "") + stereo + (nH > 1 ? "H" + nH : nH == 1 ? "H" : "") + "]");
}, "~N,~N,~N,~N,~N,~B,~S");
...
Clazz_declarePackage ("JS");
Clazz_load (["java.util.Hashtable"], "JS.SmilesParser", ["java.lang.Character", "JU.Lst", "$.PT", "$.SB", "JS.InvalidSmilesException", "$.SmilesAtom", "$.SmilesBond", "$.SmilesMeasure", "$.SmilesSearch", "JU.Elements", "$.Logger"], function () {
c$ = Clazz_decorateAsClass (function () {
this.isSmarts = false;
...
if (molecule.isTopology && (atom.hasSubpattern || atom.elementNumber != -2 || atom.nAtomsOr > 0 || atom.nPrimitives > 0)) molecule.isTopology = false;
atom.setBondArray ();
